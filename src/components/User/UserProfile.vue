<template>
  <section class="UserProfile md:p-4 p-2">
    <SwitchLang />
    <WelcomeMassage name="User" />
    <div
      class="UserProfile-content px-2 grid lg:grid-cols-2 grid-cols-1 justify-center items-center gap-4"
    >
      <div>
        <div v-if="!person.imageUrl" class="">
          <label
            for="file-input"
            class="bg-white rounded-[20px] flex justify-center items-center font-bold cursor-pointer h-[300px] w-full lg:mb-28"
          >
            {{ $t("Add images") }}
          </label>
          <input
      ...
      <div class="Form-user bg-white h-auto rounded-lg p-4 m-0">
